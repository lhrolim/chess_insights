...
describe("parseRelevantDataFromPGN", () => {
  ...
  it("should parse relevant data from PGN, playing as black", () => {
    ...
    const result = PGNParserUtil.parseGameMetadata(pgn, false);
  ...
  it("should return null if unable to parse game PGN", () => {
    ...
    const result = PGNParserUtil.parseGameMetadata(pgn, amIPlayingAsWhite);
...
describe("parseMovesFromPGN", () => {
  it("should parse moves from PGN", () => {
    const result = PGNParserUtil.parseMovesFromPGN(PGN_1);
  ...
  it("should parse moves from PGN adding move times", () => {
    const result = PGNParserUtil.parseMovesFromPGN(PGN_1);
    expect(result).toBeDefined();
    expect(result.map(m => m.move)).toContain("e2e4");
    expect(result.map(m => m.cumulativeStartPos)).toContain("e2e4");
    const clocks = result.map(m => m.timeTook);
    expect(result.map(m => m.cumulativeStartPos)).toContain("e2e4");
    expect(clocks[0]).toBe(0);
    expect(clocks[1]).toBe(300);
    expect(clocks[2]).toBe(1300);
    const noTimeArray = result.filter(m => m.timeTook === undefined);
    expect(noTimeArray.length).toBe(0);
    expect(result.length).toBe(36);
  });
});
