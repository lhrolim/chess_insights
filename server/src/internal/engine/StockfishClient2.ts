...
export class StockfishClient {
  ...
  private connect(): void {
    this.client = net.createConnection({ port: this.port, host: this.host }, () => {
      ...
      this.retryCount = 0;
      // Connection-specific initialization can go here
    });
    ...
    this.client.on("data", data => {
      ...
      if (this.resolveAnalysis) {
        this.resolveAnalysis(line);
      }
      //   if (this.resolveAnalysis && data.toString().includes("bestmove")) {
      //   } else {
      //     // console.log(`ignoring data: ${data.toString()}`);
      //   }
    });
  ...
  public returnMoveCandidates(moves: string[], depth: number = 20, lines: number = 3): Promise<MoveAnalysis[]> {
    return new Promise(resolve => {
      ...
      const onDataHandler = (data: Buffer) => {
        const output = data.toString().trim();
        const outputLines = output.split("\n");
        const filteredLines = outputLines.filter(l => UCIUtil.matchesDepth(l, depth));
        for (const line of filteredLines) {
          ...
          this.doAnalyzeStockfishLine(depth, line, moveAnalyses);
        }
